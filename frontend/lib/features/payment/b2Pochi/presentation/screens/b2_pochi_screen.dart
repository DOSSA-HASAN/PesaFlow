...
class B2PochiScreen extends ConsumerStatefulWidget {
  const B2PochiScreen({super.key});

  ConsumerState<B2PochiScreen> createState() => _B2PaybillScreenState();
}

class _B2PaybillScreenState extends ConsumerState<B2PochiScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final b2PochiRef = ref.watch(b2PochiProvider);
    ref.listen<AsyncValue<bool>>(b2PochiProvider, (previous, next) {
      if (next is AsyncData && next.value == true) {
        ...
        ToastUtil.showGeneralToast(
          ...
          title: "B2Pochi Payment Success",
          description:
          "B2Pochi payment has been submitted and is now being processed.",
        );
      } else if (next is AsyncError) {
        ToastUtil.showGeneralToast(
          context: context,
          type: ToastificationType.error,
          title: "B2Pochi Payment Failed",
                    ...
                    Expanded(
                      child: Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            Text(
                              "Business To B2Pochi Transaction",
                              style: TextStyle(
                            ...
                            const SizedBox(height: 20),
                            CustomButton(
                              label: b2PochiRef is AsyncLoading
                                  ? "Processing payment..."
                                  : "B2Pochi",
                              height: 50,
                              onPressed: b2PochiRef is AsyncLoading
                                  ? () {}
                                  : () {
                                //TODO: shortcode is hardcoded for now
                                ref
                                    .read(b2PochiProvider.notifier)
                                    .submitB2Pochi(
